...
```{r}
# Converts var_string to a numeric variable
var_num <- as.numeric(var_string)

# Checks whether var_num is even or odd
var_even <- (var_num %% 2 == 0)

# Checks whether var_num is positive or negative
var_pos <- (var_num > 0)
...
if (var_seconds == 1) {
  sprintf("%d second is %d minutes and %d second.", var_seconds, minutes, seconds)
} else if (minutes == 1 && seconds == 1) {
  sprintf("%d seconds is %d minute and %d second.", var_seconds, minutes, seconds)
} else if (minutes == 1) {
  sprintf("%d seconds is %d minute and %d seconds.", var_seconds, minutes, seconds)
} else if (seconds == 1) {
  sprintf("%d seconds is %d minutes and %d second.", var_seconds, minutes, seconds)
} else {
  sprintf("%d seconds is %d minutes and %d seconds.", var_seconds, minutes, seconds)
}
```
...
if (num1 >= num2 && num1 >= num3) {
  sprintf("The largest of the 3 numbers is: %d.", num1)
} else if (num3 >= num1 && num3 >= num2) {
  sprintf("The largest of the 3 numbers is: %d.", num3)
} else {
  sprintf("The largest of the 3 numbers is: %d.", num2)
}
```
...
num_for_prime <- 89
divisible <- FALSE
for (i in 2:as.integer(num_for_prime/2)) {
  if (num_for_prime %% i == 0) {
    divisible <- TRUE
    break
  }
}
...
if (divisible == TRUE) {
  sprintf("%s is not a prime number.", num_for_prime)
} else {
  sprintf("%s is a prime number.", num_for_prime)
}
```
...
for (i in 1:num_for_square) {

  if (i^2 > num_for_square) {
  
    first_int_exceeding_num <- i
    first_int_exceeding_num_squared <- i^2
    
    break
  }
}

sprintf("%d squared is %d, and is the first number whose square exceeds %d.", first_int_exceeding_num, first_int_exceeding_num_squared, num_for_square)
```
